...
func init() {
	rpcSever.Register(new(Arith))
	rpcSever.Register(BuiltinTypes{})
}
...
func TestMalformedInput(t *testing.T) {
	cli, srv := net.Pipe()
	go cli.Write([]byte(`{id:1}`))
	rpcSever.ServeConn(srv)
}
...
func TestUnexpectedError(t *testing.T) {
	...
	rpcSever.ServeConn(srv)                                           // must return, not loop
}
